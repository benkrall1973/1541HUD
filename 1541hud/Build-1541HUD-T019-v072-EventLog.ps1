...
$PluginSource=Join-Path $PluginDir "src\1541hud_probe_t019_v072_event_log.c"
...
$OutBase=Join-Path $BuildDir "1541HUD_OneROM_T0.0.19_v0.7.2_EventLog"
$PreservedPlugin=Join-Path $BuildDir "1541hud_probe_t019_v072_event_log_SOURCE.c"
...
Write-Host "Building T0.0.19 USER plugin..."
& wsl bash -lc "cd $RepoQ && make -C plugins/user/1541hud-probe clean && make -C plugins/user/1541hud-probe TOOLCHAIN=$ToolchainQ PLUGIN_SRC=src/1541hud_probe_t019_v072_event_log.c"
if($LASTEXITCODE -ne 0){throw "T0.0.19 USER plugin build failed"}
...
if($LASTEXITCODE -ne 0){throw "T0.0.19 USB SYSTEM plugin build failed"}
...
Write-Host "T0.0.19 build complete."
Write-Host "Expected native log records: EVENT MOTOR=0/1, EVENT TRACK=<n>, EVENT HOME=1"
