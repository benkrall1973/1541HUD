...
$T008Rel       = "plugins/user/1541hud-probe/src/1541hud_probe_t008.c"
...
$Build008 = Join-Path $Repo "1541hud/Build-1541HUD-T008.ps1"
...
$pre = Join-Path $Repo "build-1541hud/T0.0.8_SECTOR0_QUALIFY_PREP_$stamp"
New-Item -ItemType Directory -Force -Path $pre | Out-Null
git -C $Repo status --short | Set-Content -Encoding UTF8 (Join-Path $pre "pre-t008-git-status.txt")
git -C $Repo diff --binary | Set-Content -Encoding UTF8 (Join-Path $pre "pre-t008-working-tree.patch")
git -C $Repo diff --cached --binary | Set-Content -Encoding UTF8 (Join-Path $pre "pre-t008-staged.patch")
...
'PLUGIN_SRC := src/1541hud_probe_t008.c' `
"Makefile source"
...
InsertAfterLine $T008 '^#define\s+RESET_PIO0\s+\(1u\s*<<\s*11\)\s*$' @'
#define TIMER0_BASE 0x400B0000u
#define TIMER0_RAWL (*(volatile uint32_t *)(TIMER0_BASE + 0x28u))
'@ "timer"

InsertAfterLine $T008 '^#define\s+RAM_LWPT\s+0x001Eu\s*$' `
...
('$1'+$usbBlock+'}else{') "USB T0.0.8 Sector-0 formatter"

Copy-Item $Build031 $Build008 -Force
$b=Get-Content $Build008 -Raw
$b=$b.Replace("V0.0.31","T0.0.8")
$b=$b.Replace("V031","T008")
$b=$b.Replace("1541HUD_OneROM_V0.0.31","1541HUD_OneROM_T0.0.8")
[System.IO.File]::WriteAllText($Build008,$b,(New-Object System.Text.UTF8Encoding($false)))

$probe=Get-Content $T008 -Raw
$usb=Get-Content $UsbMain -Raw
$mk=Get-Content $Makefile -Raw

$checks=@(
 @{ok=($probe -match '0,0,7,0');n="T0.0.8 metadata"},
 @{ok=($probe -match 'RAM_HDRSEC\s+0x0019u');n='$0019 observed'},
 @{ok=($probe -match 'queue_sector0_qual');n="sector0 qualified helper"},
 @{ok=($usb -match 'SECTOR0 T0\.0\.8 T=');n="USB output"},
 @{ok=($mk -match '1541hud_probe_t008\.c');n="isolated source"},
 ...
 @{ok=($probe -match '\*last_track_write\s*>=\s*1u');n="raw nonzero $0022 qualification"},
...
Write-Host ""
Write-Host "Building T0.0.8..."
& powershell.exe -ExecutionPolicy Bypass -File $Build008
if($LASTEXITCODE -ne 0){throw "Build failed"}

$dir=Join-Path $Repo "build-1541hud"
$uf2=Join-Path $dir "1541HUD_OneROM_T0.0.8.uf2"
$bin=Join-Path $dir "1541HUD_OneROM_T0.0.8.bin"
...
Write-Host ""
Write-Host "Preserving exact T0.0.8 source/build..."
& powershell.exe -ExecutionPolicy Bypass -File $Preserve -TestName "T0.0.8_QUALIFIED_SECTOR0"
if($LASTEXITCODE -ne 0){throw "Preservation failed. Do not flash."}

Write-Host ""
Write-Host "T0.0.8 READY"
Write-Host "UF2: $uf2"
Write-Host "Expected:"
Write-Host "  SECTOR0 T0.0.8 T=18 US=..."
Write-Host '  [secondary $0022=0 Sector-0 writes should be suppressed]'
Write-Host ""
Write-Host "Only $0019==0 while raw current $0022 is nonzero produces diagnostic traffic."
