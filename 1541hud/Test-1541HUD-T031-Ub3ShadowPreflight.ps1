[CmdletBinding()]
param(
  [Parameter(Mandatory=$true)][string]$OneRomCli,
  [string]$Shadow = "1541hud\build-address-test\JiffyDOS-1541-6.00-shadow-address-10.bin",
  [ValidateRange(8,11)][int]$Address = 10,
  [string]$OutPlan = "1541hud\build-address-test\T0.0.31-UB3-shadow-preflight.json"
)
$ErrorActionPreference = "Stop"
if (!(Test-Path -LiteralPath $OneRomCli -PathType Leaf)) { throw "onerom.exe not found: $OneRomCli" }
if (!(Test-Path -LiteralPath $Shadow -PathType Leaf)) { throw "Shadow file not found: $Shadow" }
$shadowPath = (Resolve-Path -LiteralPath $Shadow).Path
$shadowBytes = [IO.File]::ReadAllBytes($shadowPath)
if ($shadowBytes.Length -ne 8192) { throw "Refusing: shadow must be exactly 8192 bytes." }
$patchOffset = 0x0B3A
$expected = @{
...
for ($i = 0; $i -lt 5; $i++) {
  if ($actual[$i] -ne $expected[$i]) {
    ...
    throw ('Refusing: shadow patch at CPU $EB3A does not match address ' + $Address + '. Got: ' + $got)
  }
}
$inspect = & $OneRomCli inspect slots 2>&1
if ($LASTEXITCODE -ne 0) { throw ("onerom.exe inspect slots failed:" + [Environment]::NewLine + ($inspect -join [Environment]::NewLine)) }
$inspectText = $inspect -join [Environment]::NewLine
if ($inspectText -notmatch "Configured with") { throw "Unrecognised inspect-slots output; refusing to continue." }
if ($inspectText -notmatch "JIFFYDOS") { throw "No JIFFYDOS slot was found in UB3 current configuration. Nothing changed." }
if ($inspectText -notmatch "1541 ONEROM BOOTLOADER") { throw "Expected Universal Bootloader slot was not found. Nothing changed." }
$hash = (Get-FileHash -Algorithm SHA256 -LiteralPath $shadowPath).Hash.ToLowerInvariant()
$plan = [ordered]@{
...
Write-Host ('Patch         : CPU $EB3A = ' + $plan.patch_bytes + ' (address ' + $Address + ')')
