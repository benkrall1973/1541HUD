...
$PluginSource=Join-Path $PluginDir "src\1541hud_probe_t017_v072_logging.c"
$BaselineSource=Join-Path $PluginDir "src\1541hud_probe_t016_v072_baseline.c"
$UsbSource=Join-Path $UsbDir "src\usb_main_t016_1541hud_baseline.c"
$Companion=Join-Path $BuildDir "1541hud_companion_8k.bin"
$OutBase=Join-Path $BuildDir "1541HUD_OneROM_T0.0.17_v0.7.2_Logging"
$PreservedPlugin=Join-Path $BuildDir "1541hud_probe_t017_v072_logging_SOURCE.c"
$PreservedBaseline=Join-Path $BuildDir "1541hud_probe_t016_v072_baseline_DEPENDENCY.c"
$PreservedUsb=Join-Path $BuildDir "usb_main_t016_1541hud_baseline_SOURCE.c"
New-Item -ItemType Directory -Path $BuildDir -Force | Out-Null
foreach($required in @($PluginSource,$BaselineSource,$UsbSource)){ if(!(Test-Path -LiteralPath $required)){throw "Required source missing: $required"} }
Copy-Item $PluginSource $PreservedPlugin -Force
Copy-Item $BaselineSource $PreservedBaseline -Force
Copy-Item $UsbSource $PreservedUsb -Force
[byte[]]$bytes=New-Object byte[] 8192; for($i=0;$i -lt $bytes.Length;$i++){$bytes[$i]=0xFF}; [IO.File]::WriteAllBytes($Companion,$bytes)

Write-Host "1541HUD T0.0.17 - OneROM v0.7.2 native logging experiment"
Write-Host "T0.0.16 monitor implementation remains unchanged; wrapper adds one native LOG_WRITE startup record."
...
Write-Host "Building T0.0.17 USER plugin..."
& wsl bash -lc "cd $RepoQ && make -C plugins/user/1541hud-probe clean && make -C plugins/user/1541hud-probe TOOLCHAIN=$ToolchainQ PLUGIN_SRC=src/1541hud_probe_t017_v072_logging.c"
if($LASTEXITCODE -ne 0){throw "T0.0.17 USER plugin build failed"}
Write-Host "Building v0.7.2 USB plugin with preserved 1541HUD CDC bridge and stock usb_log.c..."
...
if($LASTEXITCODE -ne 0){throw "T0.0.17 USB SYSTEM plugin build failed"}
...
Write-Host "T0.0.17 build complete."
Write-Host "Expected native log record: 1541HUD T0.0.17 v0.7.2 native LOG_WRITE startup test"
Write-Host "Expected legacy STATE/SYNC identity remains T0.0.16 by design for this isolated experiment."
Get-Item "$OutBase.bin","$OutBase.uf2",$PreservedPlugin,$PreservedBaseline,$PreservedUsb | Format-Table Name,Length,LastWriteTime
Get-FileHash "$OutBase.bin","$OutBase.uf2",$PreservedPlugin,$PreservedBaseline,$PreservedUsb -Algorithm SHA256
