...
$PluginSource=Join-Path $PluginDir "src\1541hud_probe_t018_v072_live_motor_log.c"
...
$OutBase=Join-Path $BuildDir "1541HUD_OneROM_T0.0.18_v0.7.2_LiveMotorLog"
$PreservedPlugin=Join-Path $BuildDir "1541hud_probe_t018_v072_live_motor_log_SOURCE.c"
...
Write-Host "1541HUD T0.0.18 - OneROM v0.7.2 native live_motor_log experiment"
...
Write-Host "Building T0.0.18 USER plugin..."
& wsl bash -lc "cd $RepoQ && make -C plugins/user/1541hud-probe clean && make -C plugins/user/1541hud-probe TOOLCHAIN=$ToolchainQ PLUGIN_SRC=src/1541hud_probe_t018_v072_live_motor_log.c"
if($LASTEXITCODE -ne 0){throw "T0.0.18 USER plugin build failed"}
...
if($LASTEXITCODE -ne 0){throw "T0.0.18 USB SYSTEM plugin build failed"}
...
Write-Host "T0.0.18 build complete."
Write-Host "Expected native log records: 1541HUD T0.0.18 LIVE MOTOR=0/1"
