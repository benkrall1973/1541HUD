...
$PluginSource=Join-Path $PluginDir "src\1541hud_probe_t020_v072_bounded_event_log.c"
...
$OutBase=Join-Path $BuildDir "1541HUD_OneROM_T0.0.20_v0.7.2_BoundedEventLog"
$PreservedPlugin=Join-Path $BuildDir "1541hud_probe_t020_v072_bounded_event_log_SOURCE.c"
...
Write-Host "1541HUD T0.0.20 - OneROM v0.7.2 native bounded_event_log experiment"
...
Write-Host "Building T0.0.20 USER plugin..."
& wsl bash -lc "cd $RepoQ && make -C plugins/user/1541hud-probe clean && make -C plugins/user/1541hud-probe TOOLCHAIN=$ToolchainQ PLUGIN_SRC=src/1541hud_probe_t020_v072_bounded_event_log.c"
if($LASTEXITCODE -ne 0){throw "T0.0.20 USER plugin build failed"}
...
if($LASTEXITCODE -ne 0){throw "T0.0.20 USB SYSTEM plugin build failed"}
...
Write-Host "T0.0.20 build complete."
Write-Host "Expected native log records: 1541HUD T0.0.20 LIVE MOTOR=0/1"
Write-Host "Expected legacy STATE/SYNC identity remains T0.0.16; use GUI Reconnect Test 02."
