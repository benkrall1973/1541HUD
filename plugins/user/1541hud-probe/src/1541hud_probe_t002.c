...
void drivehud_probe_main(ora_lookup_fn_t lookup,
 ...
 uint32_t raw_read_count = 0u;
 uint32_t read_dma_last_report = 0u;
 ...
 /* READDMA_PRODUCER_DIAGNOSTIC */
 if ((sector_produced_total - read_dma_last_report) >= 64u) {
     read_dma_last_report = sector_produced_total;
     queue_event(m,
         (DRIVEHUD_EVENT_READ_DMA << 28) |
         (sector_produced_total & 0x0FFFFFFFu));
 }
