/* 1541HUD T0.0.20 OneROM v0.7.2 LIVE MOTOR LOG EXPERIMENT.
 * Isolated copy of T0.0.16: only ORA log initialization and a 1 Hz
 * post-drain motor snapshot are added. The mailbox path is unchanged.
 */
...
ORA_DEFINE_USER_PLUGIN(drivehud_probe_main, 0,0,18,0, 0,7,2);

static const char t018_log_name[] = "1541HUD";
static const char t018_motor_on[] = "1541HUD T0.0.20 EVENT MOTOR=1\r\n";
static const char t018_motor_off[] = "1541HUD T0.0.20 EVENT MOTOR=0\r\n";
...
void drivehud_probe_main(ora_lookup_fn_t lookup,
 ...
 ora_log_open_write_fn_t t018_log_open;
 ora_log_write_fn_t t018_log_write;
 uint8_t t018_log_ready = 0u;
 uint32_t t018_next_log_us;
 uint8_t t020_log_motor_valid = 0u, t020_log_last_motor = 0u;
 ...
 t018_log_open = (ora_log_open_write_fn_t)lookup(ORA_ID_LOG_OPEN_WRITE);
 t018_log_write = (ora_log_write_fn_t)lookup(ORA_ID_LOG_WRITE);
 if (t018_log_open != 0 && t018_log_write != 0 &&
     t018_log_open(ORA_LOG_CHANNEL_0, t018_log_name) == ORA_RESULT_OK) {
  t018_log_ready = 1u;
 }
 t018_next_log_us = TIMER0_RAWL + 1000000u;
 ...
 /* At most one small, nonblocking application-log write per second, only
  * after draining captured samples. ORA_RESULT_LOG_FULL is intentionally
  * ignored; no retry/backpressure may disturb bus sampling. This samples
  * the same cached motor bit used by the existing USB mailbox stream. */
 if ((int32_t)(TIMER0_RAWL - t018_next_log_us) >= 0) {
  t018_next_log_us = TIMER0_RAWL + 1000000u;
  if (t018_log_ready &&
      (!t020_log_motor_valid || m->last_motor != t020_log_last_motor)) {
   t020_log_motor_valid = 1u;
   t020_log_last_motor = m->last_motor;
   if (m->last_motor != 0u) {
    (void)t018_log_write(ORA_LOG_CHANNEL_0, t018_motor_on,
                         (uint32_t)(sizeof(t018_motor_on) - 1u));
   } else {
    (void)t018_log_write(ORA_LOG_CHANNEL_0, t018_motor_off,
                         (uint32_t)(sizeof(t018_motor_off) - 1u));
   }
  }
 }
 }
