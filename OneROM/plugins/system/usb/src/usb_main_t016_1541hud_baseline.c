...
void usb_plugin_task(void) {
    ...
    // T0.0.16 baseline: preserve V0.0.32 mailbox/CDC behavior.
    drivehud_serial_task();
}

// ---------------------------------------------------------------------------
// 1541HUD T0.0.16 CDC event bridge (V0.0.32 behavior on OneROM v0.7.2)
// ---------------------------------------------------------------------------
...
static void drivehud_serial_task(void){
 ...
 static const char w[]="STATUS T0.0.16 WAITING\r\n";
 ...
 p=line;
 p=drivehud_append_str(p,e,"STATE T0.0.16 WPV=");p=drivehud_append_u32(p,e,m->write_protect_valid);
 ...
 p=drivehud_append_str(p,e,"HDRPHY T0.0.16 T=");
 ...
 p=drivehud_append_str(p,e,"RPM T0.0.16 T=");
 p=drivehud_append_u32(p,e,track);
 ...
 p=drivehud_append_str(p,e,"SYNC T0.0.16 COUNT=");
 ...
 p=line;
 p=drivehud_append_str(p,e,"STATUS T0.0.16 CAP=");p=drivehud_append_u32(p,e,m->capture_count);
